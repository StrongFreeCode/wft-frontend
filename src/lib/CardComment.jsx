...
export const CardComment = ({ excursions }) => {
  ...
  //const test = opinions ? count(2, opinions) : 'no data'
      ...
      {excursions.map((excursion, index) => (
        ...
        >
          ...
          <div className="xl:text-xl text-2xl  ">
            {
              opinions ? <MostrarEstrellas puntuacion={Rate(excursion.id)}></MostrarEstrellas> : <Spinner></Spinner>
            }

          </div>
          <div className="flex items-center justify-between">
            <span className="text-xl font-bold">
              {Rate(excursion.id) == 0 ? "0" : Rate(excursion.id) ? Rate(excursion.id).toFixed(2) : <Spinner></Spinner>} of {Reviews(excursion.id)} {language == 'es' ? 'Reseñas' : 'Reviews'}
            </span>
            <ModalComponent
